...
public class Monolith : MonoBehaviour, IUpdatable
{
    ...
    {
        foreach (KeyValuePair<System.Type, List<Monster>> kv in MonsterManager.Instance.monsterDict)
        {
            foreach (Monster m in kv.Value)
            {
                if(m.ingredientInfo.r == ingredient.r && m.ingredientInfo.g == ingredient.g && m.ingredientInfo.b == ingredient.b)
                {
                    m.ai.MonsterEnrage();
                }
            }
